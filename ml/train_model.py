...
class Dataset:
    ...
    def preprocess(self, medicine="SIB"):
        ...
        dataset["Постпрандиальная динамика лептина"] = (((dataset["Лептин 1 час (нг/мл) 0 мес"] - dataset["Лептин (нг/мл) 0 мес"]) / dataset["Лептин (нг/мл) 0 мес"])*100)
...
if __name__=="__main__":
    ...
    # 3 months
    params =  ['Возраст', 'ИМТ 0 мес', 'СРБ', 'Глюкоза', 'СКФ', 'OXC', 'САД', "Постпрандиальная динамика лептина"]
    ...
    params = ['Возраст', 'ИМТ 0 мес', 'СРБ', 'Глюкоза', 'СКФ', 'OXC', 'САД', "Постпрандиальная динамика лептина", 'ГПП 1 нг/мл 0 мес', 'ГИП (пг/мл) 0 мес', 'Грелин (нг/мл) 0 мес', 'miR142 (ПЛАЗМА) 0 мес', 'sST2 нг/мл (15,15-26,86) 0 мес']
    X, y = dt.get_X_y("SIB", 5, params=params, target_type="A")
    model = XGBClassifier()
    model.fit(X, y)
    ...
    params = ["ИМТ 3 мес", "% потери веса 3 мес", "СРБ", "САД", "Постпрандиальная динамика лептина",  'ГПП 1 нг/мл 0 мес', 'ГИП (пг/мл) 0 мес', 'Грелин (нг/мл) 0 мес', 'miR142 (ПЛАЗМА) 0 мес', 'sST2 нг/мл (15,15-26,86) 0 мес']
    X, y = dt.get_X_y("SIB", 7, params=params, target_type="both")
    model = XGBClassifier()
    model.fit(X, y)
